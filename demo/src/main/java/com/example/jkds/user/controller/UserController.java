...
public class UserController {
	...
	public ResultDto<?> authorization(@RequestBody LoginDto loginDto) {
		String token = userService.authentication(loginDto.getUserId(), loginDto.getUserPassword());
        if(token != null) {
        	return new ResultDto().makeResult(HttpStatus.OK, "로그인 성공", "Bearer "+token, "accessToken");
        } else {
